...
forecast_12_hours.each_with_index do |hour_data, index|
  ...
  precip_array.push([index + 1, (precipitation_probability * 100).round])
...
# build a chart
chart_data = forecast_12_hours.each_with_index.map do |hour_data, index|
  [index + 1, (hour_data['precipProbability'] * 100).round] # Scaling to percentage
end
